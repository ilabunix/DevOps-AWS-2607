...
# FedMail currently pulls all parts from the latest daily run.
FLB_MULTIPART_FAMILIES = {"FedMail"}
...
SFDM_PARTS = {
    ...
    "WebServices": 1,
    "ActiveWebSubscribers": 1,
    "FedAch": 1,
    "WebSubscriberServices": 1,
    "Organization": 1,
    "ServiceProvider": 1,
    "ServerCert": 1,
    "ServerCertServices": 1,
    "OrganizationSettlement": 1,
    "VpnData": 1,
    "FedlineServices": 1,
    "EasyMonthlyBilling": 13,
}
...
def show_identity(session, label):
    ...
    print("\n" + "=" * 100)
    print(ctext(label, Color.CYAN + Color.BOLD))
    print("=" * 100)
...
def find_flb_files(s3, requested_date):
    ...
    for family, prefix in FLB_PREFIXES.items():
        ...
        if family in FLB_MULTIPART_FAMILIES:
        ...
        selected = [
            obj
            for timestamp, obj in candidates
            if timestamp == latest_timestamp
        ]

        selected.sort(
            key=lambda x: object_basename(x["Key"])
        )

        print(
            f"  {family:<25} "
            f"{ctext(str(len(selected)), Color.GREEN)} file(s) "
...
def find_sfdm_files(s3, requested_date):
    ...
    for family, requested_part in SFDM_PARTS.items():

        regex = re.compile(
            rf"^CpsToSF-qy-compass-"
            rf"{re.escape(family)}-"
            rf"({ymd}\d{{6}})"
            rf"-part{requested_part}\.csv$",
            re.IGNORECASE,
        )
        ...
        if not candidates:
            ...
            print(
                ctext(
                    f"  {family:<30} "
                    f"part{requested_part:<3} MISSING",
                    Color.YELLOW,
                )
            )

            missing.append(
                f"SFDM/{family}/part{requested_part}"
            )

            continue

        latest_timestamp, latest_obj = max(
            candidates,
            key=lambda x: x[0],
        )
...
def find_dataplatform_files(s3, requested_date):
    ymd = requested_date.strftime("%Y%m%d")
    ...
    for obj in objects:
        ...
        match = ack_regex.match(filename)
    ...
    if ack_candidates:
        ...
        print(
            ctext(
                f"  {'Compass_Extract-FR ACK':<30} MISSING",
                Color.YELLOW,
            )
        )

        missing.append(
            "DATAPLATFORM/Compass_Extract-FR ACK"
        )
...
def find_wpo_files(s3):
    ...
    for family, pattern in WPO_PATTERNS.items():
...
def display_manifest(files, missing):
    ...
    for interface, family in sorted(grouped):
    ...
    if missing:
        ...
        print(
            ctext(
                "\nWARNING - EXPECTED FEEDS NOT FOUND:",
...
def download_files(s3, files, staging_root):
...
def display_download_summary(downloaded):
...
def upload_files(uat_s3, downloaded):
...
def validate_uploads(uat_s3, uploaded):
...
def display_uat_summary(uat_s3, uploaded):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compass PROD to UAT file transfer"
    )

    parser.add_argument(
        "--date",
        required=True,
        help="QA processing date YYYY-MM-DD",
    )
    ...
    print(
        ctext(
            "COMPASS PARALLEL TESTING FILE TRANSFER",
            Color.CYAN + Color.BOLD,
        )
    )
    ...
    try:
        ...
        prod_session = create_session(
            PROD_PROFILE,
            PROD_REGION,
        )

        uat_session = create_session(
            UAT_PROFILE,
            UAT_REGION,
        )
    ...
    if not confirm(
        "Confirm these PROD and UAT AWS identities are correct?"
    ):
    ...
    try:
        ...
        discovered, missed = find_flb_files(
            prod_s3,
            requested_date,
        )
        ...
        discovered, missed = find_sfdm_files(
            prod_s3,
            requested_date,
        )
        ...
        discovered, missed = find_dataplatform_files(
            prod_s3,
            requested_date,
        )
        ...
        discovered, missed = find_wpo_files(
            prod_s3,
        )
    ...
    if not files:
        ...
        print(
            ctext(
                "\nNo files were selected. Nothing to transfer.",
                Color.RED + Color.BOLD,
            )
        )
    ...
    if missing:
        ...
        print(
            ctext(
                "\nWARNING: One or more configured feeds are missing.",
                Color.YELLOW + Color.BOLD,
            )
        )
        ...
        print(
            ctext(
                "Review the missing feeds before continuing.",
                Color.YELLOW,
            )
        )

    if not confirm(
        "Proceed with downloading EXACTLY these files from PROD?"
    ):
    ...
    if not confirm(
        "Downloads validated. Proceed with UAT upload?"
    ):

        print(
            ctext(
                "\nStopped before UAT upload.",
                Color.YELLOW,
            )
        )
    ...
    validation_passed = validate_uploads(
        uat_s3,
        uploaded,
    )
    ...
    if validation_passed:
        ...
        print(
            ctext(
                "TRANSFER COMPLETED WITH VALIDATION ERRORS",
                Color.RED + Color.BOLD,
            )
        )
